...
/*function Recorder(stream, _config){
    stream = stream || null;
    this.config             = _config || {};
    this.config.sampleBits  = this.config.sampleBits || 16; //采样数位 8, 16
    this.config.sampleRate  = this.config.sampleRate || 11025; //采样率(1/6 44100)
    ...
    this.recorder.onaudioprocess = e => {
        this.audioData.input(e.inputBuffer.getChannelData(0));
        $event.trigger("record_setVolume", [e.inputBuffer.getChannelData(0)]);
    };
...
Recorder.prototype.getSoundTime = function(cb){
    let audio = new Audio;
    audio.src = window.URL.createObjectURL(this.getBlob());
    if (this.getBlob().size == 44) {
        cb(0);
...
Recorder.prototype.upload = function(url, callback){
    let fd = new FormData();
    fd.append("audioData", this.getBlob());
    fd.append("url", url.source);
    fd.append("type", "speech");
